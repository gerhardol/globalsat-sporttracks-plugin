                ...
                foreach (GhPacketBase.TrackPoint point in train.TrackPoints)
                {
                        ...
                        activity.HeartRatePerMinuteTrack.Count > 0))
                    {
                        ...
                        if (insertPause)
                        {
                            ...
                            estimatedSec = Math.Round(pointElapsed - estimatedSec);
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    pointTimeNext = pointTime.AddSeconds((int)(estimatedSec));
                                ...
                                activity.TimerPauses.Add(new ValueRange<DateTime>(
                                    ...
                                    */
                                //activity.TimerPauses.Add(new ValueRange<DateTime>( pointTimePrev.AddMilliseconds(-pointTimePrev.Millisecond ), pointTimeNext.AddMilliseconds(-pointTimeNext.Millisecond + 1000)));
                                activity.TimerPauses.Add(new ValueRange<DateTime>(pointTimeNext.AddMilliseconds(-pointTimeNext.Millisecond), pointTimePrev.AddMilliseconds(-pointTimePrev.Millisecond)));
                                if (verbose >= 10)
                                {
                                    //TODO: Remove remark when stable
                                    activity.Notes += string.Format("Added pause from {0} to {1} (dist:{2}, elapsedSec:{3}, per:{4} {5}) ",
                                        pointTimePrev.ToLocalTime(), pointTimeNext.ToLocalTime(), dist, estimatedSec, perc, info) +
